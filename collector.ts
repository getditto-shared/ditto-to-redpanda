...
async function main() {
 ...
 // AND all data < current timestamp
  ...
  rawSubscription = ditto.store.collection(RAW_COLLECTION_NAME).find("state == 'published'").subscribe()
  ...
  productSubscription = ditto.store.collection(PRODUCT_COLLECTION_NAME).find("state == 'published'").subscribe()
